...
End each text-based reply by executing the `git status` command using a tool. Do not execute it if you are already performing other tool operations in the same turn or if the previous turn's final action was `git status`.
